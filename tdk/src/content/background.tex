...
Finally, \autoref{sec:btor2} describes the essential components of the \textsc{Btor2} language used in the implementation. For a complete description of the \textsc{Btor2} syntax, the reader is referred to the original publication~\cite{btor2}.
...
To perform this transformation, software model checking toolchains typically employ specialized frontends that translate the source code into these formal models, so the so called ``backend'' can analyze it and apply mathematically precise algorithmic reasoning to system properties in an efficient way.
