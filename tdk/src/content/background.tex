...
\todoZs{So this is what I think you have to practice: structure first, sentences after; and set up the "scene" for the reader, with all of the elements, before adding thoughts on them (see comments, let's talk about this on dc)}
...
\begin{figure}
  ...
  \caption{Simple representation of a Control-Flow Automata (CFA). \todoZs{text on fig is waay too small; if you switch the font to some kind of serif in draw io, it blends in better; also try to create a more wide layout, so it does not take up this much space; and I would put this after the first par about the CFA}}
  \label{fig:cfa}
\end{figure}
...
\section{\todo{The Hardware Model Checking Format} BTOR2 \todoZs{(or something like that - always go back to section title after section is more or less written and think about what else the title should specify)}}
...
\begin{figure}
  ...
  \caption{ Syntax of Btor2. count2.btor2 example. \todoZs{use lstlisting: \url{https://www.overleaf.com/learn/latex/Code_listing}, not a picture} }
  \label{fig:count2}
\end{figure}
...
\todoZs{I would put this whole thing into a description (\url{https://www.overleaf.com/learn/latex/Lists#The_description_environment}), where each label is "Node x" or something like that - whatever looks good and makes sense}
